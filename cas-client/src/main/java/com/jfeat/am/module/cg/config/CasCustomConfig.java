...
import com.jfeat.am.module.cg.filter.RemoteLoginFilter;
...
public class CasCustomConfig {
    ...
    /**
     * 该过滤器负责用户的认证工作
     * 这个类的作用是用户是否要检测，是否已经登录了，如果没有，跳转到CAS服务器端进行登录匹配认证
     * @return
     */
    @Bean
     ...
     */
    @Bean
    public FilterRegistrationBean remoteLoginFilter() {
        FilterRegistrationBean filterRegistration = new FilterRegistrationBean();
        filterRegistration.setFilter(new RemoteLoginFilter());
        filterRegistration.setEnabled(true);
        if (autoconfig.getAssertionFilters().size() > 0) {
            filterRegistration.setUrlPatterns(autoconfig.getAssertionFilters());
        } else {
